...
{
public:
	void *ptrInQueue;					//used for LC
	unsigned int idxInPQ;			//used for PriorityQueue (may remove)

public:
	
	Interval(){ptrInQueue = NULL; idxInPQ = -1; };
	~Interval(){};
...
//wxn, not very clear why
inline void Interval::find_closest_point(double const rs, //x 
										 double const hs,// y
										 double& r, //offset, position on the edge
										 double& d_out)			//find the point on the interval that is closest to the point (alpha, s)
	{
		...
		double hc = -m_pseudo_y;// location of peudo point
...
#endif //GEODESIC_ALGORITHM_EXACT_ELEMENTS_ELEMENTS

...
struct YXPoint3D{
	...
	YXPoint3D operator-(const YXPoint3D & o) const {return YXPoint3D(x-o.x, y-o.y, z-o.z);};
	YXPoint3D operator*(double o) const { return YXPoint3D(x * o, y * o, z * o); };
...
public:
	...
	int load_m(const char * filename){
	...
	int load_obj(const char * filename){
		vert.clear(); face.clear(); nface = nvert = 0;
		FILE * fp = fopen(filename, "r"); if (fp == NULL) return -1;
		char buf[4096]; YXPoint3D vx; YXFace fc; 
		while (fgets(buf, 4096, fp) != NULL){
			if (strncmp(buf, "v ", 2) == 0){
				sscanf(buf, "v %f %f %f", &vx.x, &vx.y, &vx.z);
				++nvert; vert.push_back(vx);

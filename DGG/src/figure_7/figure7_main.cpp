...
void computeDestFanVert(CRichModel& model, const string& input_obj_name,
                    const double eps_vg, const int source, 
                    int& dest1, int& fan1_vert1, int& fan1_vert2, 
                    int& dest2, int& fan2_vert1, int& fan2_vert2)
{
  double const_for_theta = 2;
  double theta = asin(sqrt(eps_vg));
  theta *= const_for_theta;
 ...
 // for (int i = 0; i < 5; ++i) {
  ...
  //fan1_p1 = body_parts_with_angle[p1.begin_pos].p;
  //fan1_p2 = body_parts_with_angle[p1.end_pos].p;
  fan1_vert1 = body_parts_with_angle[p1.begin_pos].dest_index;
  fan1_vert2 = body_parts_with_angle[p1.end_pos].dest_index;

  int result_pos2 = (result_pos1 + body_parts_with_angle.size() / 4) % body_parts_with_angle.size();
  //while(!que.empty() ){
  //  if ( abs(que.top().second - result_pos2) < 10) {
  //    break;
  //  }
  ...
  auto p = body_parts_with_angle[result_pos2];
  dest2 = p.dest_index;
  printf("sz %d origin_pos %d begin_pos %d end_pos %d\n" , body_parts_with_angle.size(),result_pos2, p.begin_pos, p.end_pos);
  printf("dis %lf\n" , p.dest_dis);
  //fan2_p1 = body_parts_with_angle[p.begin_pos].p;
  //fan2_p2 = body_parts_with_angle[p.end_pos].p;
  fan2_vert1 = body_parts_with_angle[p.begin_pos].dest_index;
  fan2_vert2 = body_parts_with_angle[p.end_pos].dest_index;
...
void figure7_a()
{
  ...
  CylinderPath cylinder1(cylinder_radius);
	...
	printBallToObj(source_pts, "bunny_source.obj", 0.0012);
  vector<CPoint3D> dest_pts;
	for (auto& v:dest_verts) {
		dest_pts.push_back(model.Vert(v));
	}
	printBallToObj(dest_pts, "bunny_dgg_edges_pts.obj", 0.0008);
...
void figure7_b_2()
{
  ...
  int fan2_v1, fan2_v2;
  double eps_vg = 0.005;
  double cylinder_radius = 0.00035;
  computeDestFanVert(model, model_filename,eps_vg,source,dest1,fan1_v1,fan1_v2,dest2,fan2_v1,fan2_v2);
  CylinderPath cylinder1(cylinder_radius);
  cylinder1.addGeodesicPath(model,source,dest1);
  cylinder1.addGeodesicPath(model,source,fan1_v1);
  cylinder1.addGeodesicPath(model,source,fan1_v2);
  printf("s %d dest1 %d fan1_v1 %d fan1_v2 %d\n" , source, dest1, fan1_v1, fan1_v2);
  cylinder1.write_to_file("bunny_fan1_edge.obj");
  CylinderPath cylinder2(cylinder_radius);
  cylinder2.addGeodesicPath(model,source,dest2);
  cylinder2.addGeodesicPath(model,source,fan2_v1);
  cylinder2.addGeodesicPath(model,source,fan2_v2);
  cylinder2.write_to_file("bunny_fan2_edge.obj");
  ...
  dest_verts.push_back(model.Vert(dest1));
  dest_verts.push_back(model.Vert(dest2));
  dest_verts.push_back(model.Vert(fan1_v1));
  dest_verts.push_back(model.Vert(fan1_v2));
  dest_verts.push_back(model.Vert(fan2_v1));
  dest_verts.push_back(model.Vert(fan2_v2));

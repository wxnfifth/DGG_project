...
CMesh::CMesh(CMesh* pMesh)
{
	...
	m_pVertex = new CVertex[m_nVertexCapacity];			//点表
	UINT i;
	...
	m_pEdge = new CEdge[m_nEdgeCapacity]; 				//边表
	for(i=0;i<m_nEdge;i++)
		m_pEdge[i] = pMesh->m_pEdge[i];

	m_nFace = pMesh->m_nFace;	 				//面数
	m_pFace = new CFace[m_nFaceCapacity];				//面表
	for(i=0;i<m_nFace;i++)
...
// 	fout<<"# faces: "<<m_nFace<<endl;
	...
	for (UINT i = 0; i < m_nVertex; ++i) {
		...
		fout<<"v " << std::setprecision(20) <<v.x<<" "<<v.y<<" "<<v.z<<endl;
		++idx;
	}
...
void CMesh::flip(unsigned iEdge)
{
	...
	// reset face
	m_pFace[f0].m_piVertex[0] = v0; m_pFace[f0].m_piVertex[1] = v2; m_pFace[f0].m_piVertex[2] = v1;
	m_pFace[f0].m_piEdge[0] = e1; m_pFace[f0].m_piEdge[1] = e2; m_pFace[f0].m_piEdge[2] = iEdge;

	m_pFace[f1].m_piVertex[0] = v1; m_pFace[f1].m_piVertex[1] = v3; m_pFace[f1].m_piVertex[2] = v0;
	m_pFace[f1].m_piEdge[0] = e3; m_pFace[f1].m_piEdge[1] = e0; m_pFace[f1].m_piEdge[2] = twinEdge;
...
void CMesh::removeVert(unsigned iVert, unsigned startVert)
{
	// NOT FINISHED, SINCE THIS COULD BE DONE THROUGH COLLAPSE
	// Find the start vertex
	unsigned startVertIndex = -1;
	list<unsigned> polygonEdges;
	unsigned v0, v1, f0;

	for (unsigned i = 0; i < m_pVertex[iVert].m_nValence; ++i)
	{
		unsigned adjEdge = m_pVertex[iVert].m_piEdge[i];
		unsigned opEdge = m_pEdge[adjEdge].m_iNextEdge;
		unsigned prevEdge = m_pEdge[opEdge].m_iNextEdge;
		unsigned opVert = m_pEdge[adjEdge].m_iVertex[1];

		polygonEdges.push_back(opEdge);

		if (opVert == startVert)
			startVertIndex = i;

		if (m_pEdge[opEdge].m_iVertex[0] != startVert)
		{
			m_pEdge[adjEdge].m_iVertex[0] = startVert;
			v0 = m_pEdge[adjEdge].m_iVertex[0], v1 = m_pEdge[adjEdge].m_iVertex[1];
			m_pEdge[adjEdge].m_length = (m_pVertex[v0].m_vPosition - m_pVertex[v1].m_vPosition).length();

			f0 = m_pEdge[adjEdge].m_iFace;
			for (unsigned j = 0; j < 3; ++j) m_pFace[f0].m_piVertex[j] = m_pEdge[m_pFace[f0].m_piEdge[j]].m_iVertex[0];
			calFaceNormal(f0);
		}
		else
		{
			m_pFace[m_pEdge[adjEdge].m_iFace].m_bValid = false;
			if (m_pEdge[opEdge].m_iTwinEdge != -1)
				m_pEdge[m_pEdge[opEdge].m_iTwinEdge].m_iTwinEdge = m_pEdge[prevEdge].m_iTwinEdge;
			if (m_pEdge[prevEdge].m_iTwinEdge != -1)
				m_pEdge[m_pEdge[prevEdge].m_iTwinEdge].m_iTwinEdge = m_pEdge[opEdge].m_iTwinEdge;
		}

		if (m_pEdge[opEdge].m_iVertex[1] != startVert)
		{
			m_pEdge[prevEdge].m_iVertex[1] = startVert;
			v0 = m_pEdge[prevEdge].m_iVertex[0], v1 = m_pEdge[prevEdge].m_iVertex[1];
			m_pEdge[prevEdge].m_length = (m_pVertex[v0].m_vPosition - m_pVertex[v1].m_vPosition).length();

			f0 = m_pEdge[prevEdge].m_iFace;
			for (unsigned j = 0; j < 3; ++j) m_pFace[f0].m_piVertex[j] = m_pEdge[m_pFace[f0].m_piEdge[j]].m_iVertex[0];
			calFaceNormal(f0);
		}
		else
		{
			m_pFace[m_pEdge[prevEdge].m_iFace].m_bValid = false;
			if (m_pEdge[opEdge].m_iTwinEdge != -1)
				m_pEdge[m_pEdge[opEdge].m_iTwinEdge].m_iTwinEdge = m_pEdge[prevEdge].m_iTwinEdge;
			if (m_pEdge[prevEdge].m_iTwinEdge != -1)
				m_pEdge[m_pEdge[prevEdge].m_iTwinEdge].m_iTwinEdge = m_pEdge[opEdge].m_iTwinEdge;
	...
	assert(startVertIndex != -1);

	// gather polygon edges
	
	for (unsigned i = 0; i < m_pVertex[iVert].m_nValence; ++i)
	{
		if (m_pEdge[polygonEdges.front()].m_iVertex[1] == startVert) break;
		polygonEdges.push_back(polygonEdges.front());
		polygonEdges.pop_front();
	}

	unsigned endEdge = polygonEdges.back();
	unsigned startEdge = m_pEdge[polygonEdges.front()].m_iTwinEdge;

	// update startVert
	for (unsigned i = 0; i < m_pVertex[startVert].m_nValence; ++i)
	{

	}
}

void CMesh::moveVertTo(unsigned iVert, Vector3D newPos)
{
	m_pVertex[iVert].m_vPosition = newPos;
	for (unsigned i = 0; i < m_pVertex[iVert].m_nValence; ++i)
	{
		unsigned curEdge = m_pVertex[iVert].m_piEdge[i];
		unsigned twinEdge = m_pEdge[curEdge].m_iTwinEdge;
		unsigned prevEdge = m_pEdge[m_pEdge[curEdge].m_iNextEdge].m_iNextEdge;
		unsigned opVert0 = m_pEdge[curEdge].m_iVertex[1];
		unsigned opVert1 = m_pEdge[prevEdge].m_iVertex[0];

		m_pEdge[curEdge].m_length = (m_pVertex[opVert0].m_vPosition - newPos).length();
		if (twinEdge != -1) m_pEdge[twinEdge].m_length = m_pEdge[curEdge].m_length;
		if (m_pVertex[iVert].m_bIsBoundary && i == 0)
			m_pEdge[prevEdge].m_length = (m_pVertex[opVert1].m_vPosition - newPos).length();
	}
	for (unsigned i = 0; i < m_pVertex[iVert].m_nValence; ++i)
		calFaceNormal(m_pEdge[m_pVertex[iVert].m_piEdge[i]].m_iFace);
}

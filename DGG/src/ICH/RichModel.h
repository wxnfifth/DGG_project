...
public:
	...
	void Preprocess();	
    void ReadColorFile(const string &clr_file_name);
...
protected:
  ...
  vector<vector<double>> m_neighAngleSum;
	vector<bool> m_FlagsForCheckingConvexVerts;
  vector<double> m_angle_sum;
	vector<CEdge> m_Edges;
	vector<CSimpleEdge> m_SimpleEdges;
...
const CRichModel::CSimpleEdge& CRichModel::SimpleEdge(int simpleEdgeIndex) const//add by wxn
{
	return m_SimpleEdges[simpleEdgeIndex];

}

const vector<double>& CRichModel::NeighAngleSum(int root) const
{

...
public:
	Point2D() {}
	Point2D(double x, double y) {m_coordinates[0] = x; m_coordinates[1] = y;}
	Point2D(Point2D *p)
	{
		x() = p->x();
		y() = p->y();
	...
	Point2D& operator*=(const double& right)
	{
		this->x() *= right;
		this->y() *= right;
		return *this;
	...
	double* xy(){return m_coordinates;};
...
// 	}
	...
	Point3D& operator/=(const double &right) {
		this->x() /= right;
		this->y() /= right;
		this->z() /= right;
		return *this;

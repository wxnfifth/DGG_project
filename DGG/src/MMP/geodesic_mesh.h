...
public:
	...
	double max_edge() {return max_edge_len;}
	double min_edge() {return min_edge_len;}
	double avg_edge() {return avg_edge_len;}
	double max_theta() {return theta_max;}
...
private:
	...
	double max_edge_len, min_edge_len, avg_edge_len;
	double theta_min, theta_max, h_min, h_max;
};
...
inline void Mesh::build_adjacencies()
{
	...
	for(unsigned i=0; i<m_faces.size(); ++i)
	{
		...
		for(unsigned j=0; j<3; ++j)		//compute angle adjacent to the vertex j
		{
			...
			assert(angle>1e-7);						//algorithm works well with non-degenerate meshes only 

			f.corner_angles()[j] = angle;
			sum += angle;
		}
		assert(std::abs(sum - M_PI) < 1e-7);		//algorithm works well with non-degenerate meshes only 
	}
...
inline void Mesh::calculate_statics_data()
{
	max_edge_len = -1e100;
	min_edge_len = 1e100;
	avg_edge_len = 0;
	for (unsigned i = 0; i < m_edges.size(); ++i)
	{
		avg_edge_len += m_edges[i].length();
		max_edge_len = std::max(max_edge_len, m_edges[i].length());
		min_edge_len = std::min(min_edge_len, m_edges[i].length());
	}
	avg_edge_len /= m_edges.size();

	theta_min = 1e30; theta_max = -1e30;
	h_min = 1e30; h_max = -1e30;
	for (unsigned i = 0; i < m_faces.size(); ++i)
	{
		double edge[3];
		edge[0] = m_faces[i].adjacent_edges()[0]->length();
		edge[1] = m_faces[i].adjacent_edges()[1]->length();
		edge[2] = m_faces[i].adjacent_edges()[2]->length();

		for (unsigned j = 0; j < 3; ++j)
		{	
			double curCosTheta = (edge[j]*edge[j] + edge[(j+1)%3]*edge[(j+1)%3] - edge[(j+2)%3]*edge[(j+2)%3])/(2*edge[j]*edge[(j+1)%3]);
			double curTheta = acos(curCosTheta);
			double curH = edge[j] * sqrt(fabs(1-curCosTheta*curCosTheta));
			if (curTheta < theta_min) theta_min = curTheta;
			if (curTheta > theta_max) theta_max = curTheta;
			if (curH < h_min) h_min = curH;
			if (curH > h_max) h_max = curH;
		}
	}
}

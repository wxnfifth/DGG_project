...
public:
	...
	void propagate(
						 vector<pair<CPoint3D,CPoint3D>>& interval_points,
...
private:
	...
	void update_list_and_queue(std::set<int>& coverted_verts,
                                      list_pointer list,
	...
	void initialize_propagation_data(std::set<int>& covered_verts);		
...
inline void GeodesicAlgorithmExact::initialize_propagation_data(std::set<int>& covered_verts)
{
	clear();
	...
	for(unsigned i=0; i<m_sources.size(); ++i)		//for all edges adjacent to the starting vertex			
	{
		...
		for(unsigned j=0; j<edges_visible_from_source.size(); ++j)
		{
			...
			update_list_and_queue(covered_verts, interval_list(e), &candidate, 1);
		}
	}
}
...
inline void GeodesicAlgorithmExact::propagate(vector<pair<CPoint3D,CPoint3D>>& interval_points,
																							std::set<int>& covered_verts,
                                              std::vector<SurfacePoint>& sources,
   									                          double max_propagation_distance,			//propagation algorithm stops after reaching the certain distance from the source
									                            std::vector<SurfacePoint>* stop_points)
{
	set_stop_conditions(stop_points, max_propagation_distance);
	set_sources(sources);
	initialize_propagation_data(covered_verts);
	...
	while(!m_queue.empty())
	{
		...
		for(unsigned i=0; i<edge->adjacent_faces().size(); ++i)		//two possible faces to propagate
		{
			...
			if(num_propagated)
			{
				...
				update_list_and_queue(covered_verts, 
                    interval_list(next_edge), 
									  candidates, 
									  num_propagated);
			}
	...
	//interval_points
	for (auto& interval_ptr:m_queue) {
		auto& e_ptr = interval_ptr->edge();
		CPoint3D v0(e_ptr->v0()->xyz());
		CPoint3D v1(e_ptr->v1()->xyz());
		double x0 = interval_ptr->start() / e_ptr->length();
		CPoint3D p0 = (1-x0)*v0 + x0 * v1;
		double x1 = interval_ptr->stop() / e_ptr->length();
		CPoint3D p1 = (1-x1)*v0 + x1 * v1;
...
{  
  ...
  std::vector<vertex_pointer> candidate_verts;
  if (num_candidates == 1)
  {
    IntervalWithStop* first;
    first = candidates;
    //coverted_verts.insert(first->edge()->v0()->id());
    //coverted_verts.insert(first->edge()->v1()->id());
    candidate_verts.push_back(first->edge()->v0());
    candidate_verts.push_back(first->edge()->v1());
  }else{
    IntervalWithStop* first;
    ...
    //coverted_verts.insert(first->edge()->v0()->id());
    //coverted_verts.insert(first->edge()->v1()->id());
    //coverted_verts.insert(second->edge()->v0()->id());
    //coverted_verts.insert(second->edge()->v1()->id());
    candidate_verts.push_back(second->edge()->v0());
    candidate_verts.push_back(second->edge()->v1());
